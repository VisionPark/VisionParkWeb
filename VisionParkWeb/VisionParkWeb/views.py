...
from django.contrib.auth.decorators import login_required, parking_admin_required
...
def parkings(request):
    ...
    if request.method == "POST" and request.POST.get('idParkingSelected'):
        selectedParking = Parking.objects.get(
            id=request.POST.get('idParkingSelected'))

        zoom = request.POST.get('zoom')
        if selectedParking.num_spaces > 0:
            per_occupied = round(
                selectedParking.num_spaces_not_vacant/selectedParking.num_spaces * 100)
        else:
            per_occupied = 0
        ...
        if form.is_valid():
            ...
            if hidden_id is not None and hidden_id != 'None':
                ...
                stock.save(update_fields=[
                           'name', 'address', 'postcode', 'lon', 'lat', 'date_modified', 'img'])
            # Creating new parking
            else:
                stock.save()
